 ...
 * 
 * Originally by John Walley: https://github.com/johnwalley/d3-simple-slider
...
function slider(orientation, scale) {
  ...
  var listeners = d3.dispatch('onchange', 'start', 'end', 'drag');
  ...
  function slider(context) {
    ...
    identityClamped = d3.scaleLinear()
      .range(scale.range())
      .domain(scale.range())
      .clamp(true);
    ...
    function dragstarted() {
      ...
      var pos = identityClamped(
        orientation === bottom || orientation === top ? d3.event.x : d3.event.y
      );
